...
<head>
<meta charset="utf-8" />
<title>Savetakatak| Subcriber</title>
            ...
            <table class="table table-striped table-bordered table-hover dt-responsive" width="100%" id="sample_4">
              <thead>
                <tr>
                  ...
                  <th class="desktop">Subcribe Date</th>
              ...
              <tbody>
              <?php if ($userssub): ?>
                  <?php foreach ($userssub as $ct): ?>
                <tr>
                  <td><?php  echo $ct->email?></td>
                  <td><?php  echo $ct->added_date?></td>
                  <td><a class="btn btn-success inactive" href="<?php echo site_url('users/statussub/'.$ct->subc_id.'') ?>">
                        <?php if ( $ct->status==1): echo "Active" ;?>
                        <?php else: echo "InActive"; ?>
                        <?php endif ?>
                    </a></td>
                 <td><a class="delete"  href="<?php echo site_url('users/deletesub/'.$ct->subc_id.'') ?>" >  <i class="fa fa-trash"></i></a></td>
