...
class Orders extends MX_Controller{
	...
	public function addorder(){
		...
		if ($productSessionData) {
			...
			$this->session->set_flashdata('warnings','No products are added');
			redirect('orders/add');
		}
	}
	// update order
	...
	public function addorderhistroy(){
		...
		if(checkModification()){
			if ($this->form_validation->run() == FALSE){
				...
				$lastStatusId = $this->order->getLastStatusId($order_id,$sub_order_id);
					$data = array(
						'order_id'=>post('order_id'),
	...
	public function paymentupdate(){
		if (post('ajax')){
			$ret = $this->order->updatePayment();
			echo json_encode($ret);
		}
	}

	public function printinvoice($o_id){
		if ($o_id=='') {
		...
		$data['order'] = $this->order->getorder($o_id);
		$theme = $this->session->userdata('admin_theme');
		if (file_exists(APPPATH . 'views/themes/' . $theme . '/template/order/invoice')) {
			$this->load->view('themes/' . $theme . '/template/order/invoice', $data);
		}else{
			$this->load->view('themes/default/template/order/invoice', $data);
		}		
	}
	...
	public function mailtemplate(){
		...
		$theme = $this->session->userdata('admin_theme');
		if (file_exists(APPPATH . 'views/themes/' . $theme . '/template/order/mail')) {
			$this->load->view('themes/' . $theme . '/template/order/mailtemplate', $data);
		}else{
			$this->load->view('themes/default/template/order/mailtemplate', $data);
		}
	}
	public function getcustomer(){
		$this->output->set_content_type('application/json')->set_output(json_encode( $this->input->get('term')));
	}
}	
