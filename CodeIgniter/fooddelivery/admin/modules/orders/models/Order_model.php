...
class Order_model extends CI_Model {
	...
	public function record_count() {
		...
		if($payment_method){
			$this->db->like('payment_method',$payment_method);
		...
		if($date_added){
			$this->db->where('orders.created_on',date('Y-m-d',strtotime($this->input->get('date_added'))));
	...
	public function fetch_data($limit, $start){
		$this->db->select('orders.*,customer.first_name,customer.last_name,order_status.*', FALSE);
		$order_id = $this->input->get('order_id');
		$customer = $this->input->get('customer');
		$cv_status = $this->input->get('cv_status');
		$payment_method = $this->input->get('payment_type');
	...
	function fetch_data_bysearch(){
		$this->db->like('customer.first_name',post('search'), 'both');
		$this->db->or_like('customer.last_name',post('search'), 'both');
		$this->db->or_like('order_status.name',post('search'), 'both');
		$this->db->or_like('orders.payment_method',post('search'), 'both');
	...
	public function suborderTotalamt($subOrder_id){
		$s_id = $subOrder_id;
		$this->db->where('so_id', $s_id);
		$os =  $this->db->get('order_store')->row();
	...
	public function updatePayment(){
		...
		$where = array('o_id'=>$id);
		$ret = $this->db->update('orders',$data,$where);
		$this->db->update('order_store',$data,$where);
	   if ($ret) {
		...
		$result = $this->db->get('order_store')->result();

		$amt = 0;
		$totalAmt = 0;
		foreach ($result as $os) {
				$total = $os->total;
				$commission =$os->store_commission;
				$com = ($total * $commission )/100;
			if($os->payment_method =='paying-cash'){
				$amt = $com; 
			...
			$data = array(
				...
				'main_order'=>$os->o_id,
				'description'=>$type.'balance for order no:'. $id .'',
				'amount'=>$totalAmt,
				'sub_order_id'=>$os->so_id,
				'added_date'=>date('Y-m-d')
			);
		   $this->db->insert('merchant_wallet_history', $data);
		   $last_id = $this->db->insert_id();

		   $this->db->where('mwh_id', $last_id);
		   $rest = $this->db->get('merchant_wallet_history')->row();

		   $this->db->where('merchant_id',$rest->merchant_id);
		   ...
		   if($getmerchant->merchant_id){
				...
				if($rest->transaction_type=='credit'){
					 $dataUp = array(
						'merchant_balance'=>$getmerchant->merchant_balance + $rest->amount
					);
				 }else{
					 $dataUp = array(
						'merchant_balance'=>$getmerchant->merchant_balance - $rest->amount
	...
	public function getcompletedOrder(){
			...
			if ($totalOrder) {
				...
				$order_histroy = $this->db->get('order_histroy')->result();
				$comp = count($order_histroy);
				$percent = ($comp * 100 )/$totalOrder;
				return $percent;
			}else{
				return 0;
			}
