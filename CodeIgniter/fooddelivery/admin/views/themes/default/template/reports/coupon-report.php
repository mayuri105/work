...
<div class="static-content-wrapper">
	   <div class="static-content">
			 <div class="page-content">
				<ol class="breadcrumb">
					...
					<li class="active"><a href=""> Coupon Reports</a></li>
				</ol> 
				<div class="container-fluid">
					<div class="panel panel-default">
						<div class="panel-heading">
							<h3 class="panel-title"><i class="fa fa-bar-chart"></i> Coupon Reports</h3>
						</div>
						<div class="panel-body">
							<div class="well">
								<div class="row">
								
									<form action="<?= site_url('reports/coupon_report') ?>" method="get" name="filter">
	...
	<script type="text/javascript">	
        ...
        <?php foreach ($coupons as $coupon){ ?>
            <?php if($mode == 'year'){ ?>    
                data.push(["<?= date('Y', strtotime($coupon->date_start)) ?>", <?= $coupon->total_use ?>]);
            <?php }else if($mode == 'month'){ ?>
                data.push(["<?= date('F, Y', strtotime($coupon->date_start)) ?>", <?= $coupon->total_use ?>]);
            <?php }else if($mode == 'day') { ?>
                data.push(["<?= date('d/m/Y', strtotime($coupon->date_start)) ?>", <?= $coupon->total_use ?>]);
            <?php }else{ ?>
                data.push(["<?= date('d/m/Y', strtotime($coupon->date_start)).' - '.date('d/m/Y', strtotime($coupon->date_end)) ?>", <?= $coupon->total_use ?>]);
            <?php } ?>
        <?php } ?>
        ...
        $('#chart-ad').bind('plothover', function(event, pos, item) {
                ...
                if (item) {
                        $('<div id="tooltip" class="tooltip top in"><div class="tooltip-arrow"></div><div class="tooltip-inner">Total Used :' + item.datapoint[1].toFixed(2) + '</div></div>').prependTo('body');
