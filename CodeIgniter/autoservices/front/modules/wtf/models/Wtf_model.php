...
class Wtf_model extends CI_Model {

    /**
     * @name string TABLE_NAME Holds the name of the table in use by this model
     */
    const TABLE_NAME = 'wtp';

    /**
     * @name string PRI_INDEX Holds the name of the tables' primary index used in this model
     */
    const PRI_INDEX = 'wtp_id';
...
$this->db->where('registration_date <=', $registration_date_to);
    ...
    //  $this->db->order_by('claim_date','desc');
        ...
        $this->db->where('wtp_id', $id);
        $query = $this->db->get('claims_wtp')->result();
        return  count($query);
    }
    ...
    public function getclaim($id){
         ...
         $ret = $this->db->get_where('claims_wtp',array('wtp_claim_id'=>$id))->row();
         if(!$ret){
            show_404();
         } 
         return $ret;
    ...
    public function getclaimbycaresid($id){
         ...
         $ret = $this->db->get_where('wtp',array('wtp_id'=>$id))->row();
     ...
     public function get($id){
         ...
         $ret = $this->db->get_where(self::TABLE_NAME,array('wtp_id'=>$id))->row();
         if(!$ret){
            show_404();
           } 
         return $ret;
    ...
    public function insertclaim(Array $data) {
        if ($this->db->insert('claims_wtp', $data)) {
            return $this->db->insert_id();
        } else {
            return false;
        }
    ...
    public function updateclaim(Array $data, $where = array()) {
            if (!is_array($where)) {
                $where = array('wtp_claim_id' => $where);
            }
        $this->db->update('claims_wtp', $data, $where);
        return $this->db->affected_rows();
    }
   ...
   public function getlocation(){
    ...
    public function claimdelete($id) {
        ...
        $this->db->where('wtp_claim_id', $id);
    
        $this->db->delete('claims_wtp');
        return $this->db->affected_rows();
    }
}
