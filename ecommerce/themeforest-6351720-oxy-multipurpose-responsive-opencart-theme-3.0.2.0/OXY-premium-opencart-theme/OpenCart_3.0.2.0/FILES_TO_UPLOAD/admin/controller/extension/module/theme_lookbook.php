...
class ControllerExtensionModuleThemeLookBook extends Controller {
	...
	public function index() {
		$this->load->language('extension/module/theme_lookbook');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_setting_module->addModule('theme_lookbook', $this->request->post);
			} else {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/theme_lookbook', 'user_token=' . $this->session->data['user_token'], true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/theme_lookbook', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true)
			);			
		}
		
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/theme_lookbook', 'user_token=' . $this->session->data['user_token'], true);
		} else {
			$data['action'] = $this->url->link('extension/module/theme_lookbook', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		if (isset($this->request->post['lookbook'])) {
			$data['lookbook'] = $this->request->post['lookbook'];
		} elseif (!empty($module_info)) {
			$data['lookbook'] = $module_info['lookbook'];
		} else {
			$data['lookbook'] = '';
		}	
		...
		$this->response->setOutput($this->load->view('extension/module/theme_lookbook', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/theme_lookbook')) {
