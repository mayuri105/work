					...
					<div class="row">
						...
						<!-- start: PAGE TITLE & BREADCRUMB -->
							<ol class="breadcrumb">
								...
								<?php if($_GET['process']==base64_encode('addStock')) { 
									?>			
									<li>
										Add New Stock
									</li>
									<?php } if($_GET['process']==base64_encode('editStock')) { 
									?>			
									<li>
										Edit Stock Details
									</li>
								...
								<div class="panel-body">
									...
									<div class="table-responsive">
										...
										case 'addStock': ?>
											...
											<form name="addStock" action="inc/process-library.php" method="post" enctype="multipart/form-data" autocomplete="off" onSubmit="return addstock();">
												<input type="hidden" name="process" value="saveStock" />
												...
												<div class="col-sm-3 ">
													...
													?>
													</select><span  style="color:#FF6699;" id="product_err"></span>
													<br>
                                                    <input type="text" class="form-control" placeholder="Stock Quntity" name="qty" id="qty"/><span style="color:#FF6699;" id="sqty_err"></span>
													<br>
                                                    
													<div class="col-sm-3">
														<input type="submit" name="saveStock" value="Save Stock" class="btn btn-teal" style="font-family:Verdana, Arial, Helvetica, sans-serif;"/>
													</div>
										...
										<?php break;
										 case 'editStock': ?>
											...
											<input type="hidden" name="process" value="editStock" />
											<input type="hidden" name="returnUrl" value="<?=$_SERVER['REQUEST_URI'];?>" />
									 <?php
													$editdetail=new adminClass();
													$row=$editdetail->StockdetailById($_GET['sId']);
											?>
									 <div class="col-sm-3">
											...
											<select class="form-control" name="product_name">
													...
													foreach($allproduct as $allproduct)
													{	
														if($allproduct['id']==$row['product_type'])
														{?>
                                                    ...
                                                    <input type="text" class="form-control" placeholder="Stock Quntity" name="qty" id="qty"  value="<?php echo $row['qty'];?>"/><span style="color:#FF6699;" id="euname_err"></span>
													<br>
                                                   
													
											<div class="col-sm-4">
												<input type="submit" name="saveStock" value="Update Stock" class="btn btn-teal" style="font-family:Verdana, Arial, Helvetica, sans-serif;" />
											</div>
										</div>
										...
										<a href="stock.php?process=<?=base64_encode('addStock')?>"  class="btn btn-teal"  style="float:right; margin-right:2em;font-family:Verdana, Arial, Helvetica, sans-serif;">Add Stock</a>
										...
										<table class="table table-striped table-bordered table-hover" id="sample-table-2">
											...
											<tbody>
												<?php
													$adminClass=new adminClass();
													$allStock=$adminClass->stockdetail();
													$s=1;
													foreach($allStock as $allStock)
													{	
												 ?>
												<tr>
													<td><?=$s;?></td>
													<td><?=$allStock['pname'];?> </td>
													
													<td><?=$allStock['qty'];?></td>
                                                    
													<td><a data-original-title="Edit" data-placement="top" class="btn btn-xs btn-teal tooltips" href="stock.php?process=<?=base64_encode('editStock');?>&sId=<?=$allStock['id'];?>"><i class="fa fa-edit"></i></a>
													<a data-original-title="Remove" data-placement="top" class="btn btn-xs btn-bricky tooltips"  href="delete.php?skId=<?=$allStock['id'];?>" ><i class="fa fa-times fa fa-white"></i></a>
													</td>									
