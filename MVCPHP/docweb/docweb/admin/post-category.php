					...
					<div class="row">
						...
						<!-- start: PAGE TITLE & BREADCRUMB -->
							<ol class="breadcrumb">
								...
								<li class="active">
									 <a href="post-category.php"> Post Category </a>
								</li>
								
								<?php if($_GET['process']==base64_encode('addCategory')) { 
									?>			
									<li>
										Add New Post Category
									</li>
									<?php } if($_GET['process']==base64_encode('editCategory')) { 
									?>			
									<li>
										Edit Post Category
									</li>
									...
									<div class="table-responsive">
										...
										case 'addCategory': ?>
											...
											<form name="addCategory" action="inc/process-library.php" method="post" enctype="multipart/form-data">
												<input type="hidden" name="process" value="saveCategory" />
												<input type="hidden" name="returnUrl" value="<?=$_SERVER['REQUEST_URI'];?>" />
											
											
													<div class="col-sm-3">
													<input type="text" class="form-control" id="form-field-10" placeholder="Category Name" name="name">
													</div>
													<div class="col-sm-3">
														<input type="submit" name="saveCategory" value="Save Category" class="btn btn-teal" />
													</div>
										...
										<?php break;
										 case 'editCategory': ?>
											...
											<input type="hidden" name="returnUrl" value="<?=$_SERVER['REQUEST_URI'];?>" />
												...
												<?php
													$postName=new postCategoryClass();
													$catName=$postName->fetchPostCategoryById($_GET['catId']);
													 ?>
									 ...
									 <div class="form-group">
											...
											<div class="col-sm-3">
												<input type="hidden" name="catId" value="<?=$catName['id'];?>" />
												<input type="text" class="form-control" id="form-field-10"  name="name" value="<?=$catName['category_name'];?>">
											</div>
											<div class="col-sm-4">
												<input type="submit" name="saveCategory" value="Update Category" class="btn btn-teal" />
											</div>
										</div>
										...
										<a href="post-category.php?process=<?=base64_encode('addCategory')?>"  class="btn btn-teal"  style="float:right; margin-right:30em;">Add Post Category</a>
										...
										<table class="table table-striped table-bordered table-hover" id="sample-table-2">
											...
											<tbody>
												...
												<tr>
													...
													<td><a href="javascript:void(0);"  onClick="javascript:status('<?php echo $cat['id']?>','<?php echo $cat['category_status']?>');"><?php if($cat['category_status']=='0'){ echo "Active"; } else { echo "Deactivate"; } ?></a></td>
													<td><?=$cat['category_date_created'];?></td>
													<td><?=$cat['category_date_modified'];?></td>
													<td><a data-original-title="Edit" data-placement="top" class="btn btn-xs btn-teal tooltips" href="post-category.php?process=<?=base64_encode('editCategory');?>&catId=<?=$cat['id'];?>"><i class="fa fa-edit"></i></a>
													<a data-original-title="Remove" data-placement="top" class="btn btn-xs btn-bricky tooltips"  href="delete.php?catId=<?=$cat['id'];?>" ><i class="fa fa-times fa fa-white"></i></a>
													</td>									
