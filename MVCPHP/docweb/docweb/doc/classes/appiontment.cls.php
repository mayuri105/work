...
include("../config/dbconnect.php");
  class appo{
	...
	function getappoinfo(){
		$appoinfo=array("apid"=>$this->apid, "Patient_id"=>$this->Patient_id, "type"=>$this->type, "fname"=>$this->fname, "lname"=>$this->lname, "email"=>$this->email, "address"=>$this->address, "contactno"=>$this->contactno, "country"=>$this->country, "state"=>$this->state, "city"=>$this->city, "gender"=>$this->gender, "age"=>$this->age, "speciality"=>$this->speciality, "maultispeciality"=>$this->maultispeciality, "doctor"=>$this->doctor, "adate"=>$this->adate, "atime"=>$this->atime, "status"=>$this->status, "added_date"=>$this->added_date);			
		return $appoinfo;	
	}
	function addappo(){
		
			 echo $sql="insert into tbl_appointment (Patient_id, type, fname, lname, email, address, contactno, country, state, city, gender, age, speciality, maultispeciality, doctor, adate, atime) 
			values('".$_REQUEST["patid"]."','".$_REQUEST["lblApType"]."','".$_REQUEST["txtFirstName"]."','".$_REQUEST["txtLastName"]."','".$_REQUEST["txtEmail"]."','".$_REQUEST["txtAddress"]."','".$_REQUEST["txtContactno"]."','".$_REQUEST["lblCountry"]."','".$_REQUEST["lblState"]."','".$_REQUEST["lblCity"]."','".$_REQUEST["gender"]."','".$_REQUEST["txtAge"]."','".$_REQUEST["lblSpeciality"]."','".$_REQUEST["txtMultiSpec"]."','".$_REQUEST["Doctor"]."','".$_REQUEST["txtDate"]."','".$_REQUEST["lblTime"]."')"; 
			$res=mysql_query($sql);
...
AND tbl_appointment.doctor = tbl_doctor.docid AND doctor = '".$_SESSION['Login_docid']."'
";
		  $res=mysql_query($sql);
		$num=@mysql_num_rows($res);
		if($num > 0){
			$appodata=array();
			   	while($fields=mysql_fetch_array($res)){
					$appodata[]=array("apid"=>$fields["apid"],"Patient_id"=>$fields["Patient_id"],"type"=>$fields["type"],"fname"=>$fields["fname"],"lname"=>$fields["lname"],"email"=>$fields["email"],"contactno"=>$fields["contactno"],"country"=>$fields["country"],"state"=>$fields["state"],"city"=>$fields["city"],"gender"=>$fields["gender"],"age"=>$fields["age"],"speciality"=>$fields["speciality"],"maultispeciality"=>$fields["maultispeciality"],"doctor"=>$fields["doctor"],"adate"=>$fields["adate"],"atime"=>$fields["atime"],"ap_time_slots"=>$fields["ap_time_slots"],"name"=>$fields["name"],"status"=>$fields["status"]);
				}	
