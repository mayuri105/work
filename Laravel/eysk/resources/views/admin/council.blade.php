						...
						<div class="kt-portlet__head-toolbar">
							<div class="kt-portlet__head-wrapper">
								<div class="kt-portlet__head-actions">
								    @if(in_array('add-council',$accessData))
    									<a href="{{ route('add-council') }}" class="btn btn-warning btn-elevate btn-icon-sm">
    										<i class="la la-plus"></i>
    										Add Council 
    									</a>
    								@endif
    								@if(in_array('delete-all-council',$accessData))
							...
							<table id="example" class="table table-striped table-bordered table-hover">
								...
								<tbody>
									 @foreach($council as $councils) 
										<tr>
											<td class="text-center"><label class="kt-checkbox kt-checkbox--single kt-checkbox--solid"><input type="checkbox" name="sub_check_all[]" value="{{ $councils->council_id }}" class="sub_check_all" onchange="checkSubCheckbox()" id="pages" style="width: 16px;height: 16px;"><span></span></label></td>
											<td>{{ $councils->name }}</td>
											<td>{{ $councils->code }}</td>											
											<td>{{ $councils->tagsname }}</td>
											<td> @if($councils->created_at != "0000-00-00 00:00:00" && $councils->created_at != NULL){{ date("d-m-Y",strtotime($councils->created_at)) }} @endif</td>
											<td style="width: 10%;">
											    @if(in_array('edit-council',$accessData))
												    <a href="edit-council/{{ $councils->council_id }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Edit" style="font-size: 25px;"><i class="la la-edit" style="font-size: 20px;"></i></a>
												@endif
												@if(in_array('delete-council',$accessData))
												    <a href="#"  onclick="deleteFunction({{ $councils->council_id }})" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Delete" style="font-size: 25px;"><i class="fa fa-trash-o" style="font-size: 20px;"></i></a>
											    @endif
											</td>
										</tr>
									 @endforeach 
	...
	function deleteFunction(id) {
		...
		.then((willDelete) => {
			if (willDelete) {
				...
				location.href = 'delete-council/'+id;
